...
int nodecnt(struct node *root)
{
    struct node *current=root;
    int nodelf=0;
    if(current->left!=NULL)nodelf=nodelf+1+nodecnt(current->left);
    if(current->right!=NULL)nodelf=nodelf+1+nodecnt(current->right);
    if(current->left==NULL&&current->right==NULL)
    {
        return nodelf;
    }
...
for(i=0;i<n;i++)
{
 ...
 int tot=1+nodecnt(root);//one is added here becoz we r missing the root nod
 cout<<" Total Number of node in BST "<<tot<<endl;
 cout<<"Inoder traversal : ";
 inorder(root);
 cout<<endl;
 cout<<"Preoder traversal : ";
